...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Trabalho Segunda account.<br>
        ...
        &mdash; Trabalho Segunda Team
      `
      return sendMail({ toEmail: email, subject: 'Trabalho Segunda - Password Reset', content })
    })
